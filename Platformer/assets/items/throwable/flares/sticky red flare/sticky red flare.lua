...
function sticky_red_flare_ui:setProperties()
	--Used in the game
	self.showcaseName = "Sticky red flare"
	self.texture = "items/throwable/flares/sticky red flare/texture.png"
	self.icon = "items/throwable/flares/sticky red flare/icon.png"
	self.type = "Utility"
	self.itemSize = {20,5}
	self.offset = {0,0}
	self.angleOffset = 0
...
newUsableItem("sticky_red_flare_ui")

--Called when the item gets equipped
function sticky_red_flare_ui:onEquip(heldItem, mousePos)
	
end

--Called while the item is equipped
function sticky_red_flare_ui:whileEquipped(heldItem, mousePos)
	...
	if heldItem:getArmAnimationName() == "holding" then
		if input.keyPressed(heldItem:getUseButton()) then
			heldItem:createProjectile("sticky_red_flare_pr",0,0,directionVec[1] * 500, directionVec[2] * 500)
			heldItem:consume()
		end
	end
